...
The system is quite structured but there is still a part that we don't like: DocumentProcessor has to be modified 
for each type of reader that appears.

Modify the code for each reader to determine whether or not it can process a document and make it so that in 
the construction of DocumentProcessor all available readers are passed to it. The goal is that,
